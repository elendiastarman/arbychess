...
Function run(g.game)
	...
	testc.cell = fetchCellById(1)
	testm.move = testc\p\m\m
	
	DebugLog testm\dx+" : "+testm\dy
	
	;tests.celloplayer = destinations.celloplayer(test, 1, -1, 1)
	tests.celloplayer = possibleCells(testc, testm, g\b\ramifying)
...
Function movePiece(p.piece, c.cell)

End Function

Function possibleCells.celloplayer(c.cell, m.move, ramifying)

	;for move, destination cells
	;for each destination cell, further moves
	;recurse
	
	dests.celloplayer = destinations.celloplayer(c, m\dx, m\dy, ramifying)
	;destsb.celloplayer = New celloplayer
	;append.celloplayer = New celloplayer
	
	;destsb = dests
	
	DebugLog "dests"
	While dests <> Null
		DebugLog "dId "+dests\c\id
	
		While m\nextMoves <> Null
			DebugLog "mId "+m\nextMoves\m\id
		
			dests2.celloplayer = possibleCells.celloplayer(dests\c, m\nextMoves\m, m\nextMoves\m, ramifying)
