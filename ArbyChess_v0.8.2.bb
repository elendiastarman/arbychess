...
Type historyList
	...
	Field srcCell.cell, destCell.cell ;cell moved from
...
Function undo(b.board)
	...
	If hist\bef <> Null
		...
		If hist\capId > 0
			;hist\capCell\p = copyPiece( fetchPieceById( hist\capId ) )
			DebugLog hist\capP\id
...
Function removeMove2(ml.movelist, moveId)

	hold.move = ml\m
	success = 0
	
	If hold\id = moveId ;edge case that you're removing the one you're on
		If ml\bef <> Null
			ml\bef\nex = ml\nex
		EndIf
		If ml\nex <> Null
	...
	While ml\bef <> Null ;rewind to beginning
		ml = ml\bef
	Wend

	While ml <> Null ;step through the list forward
		If ml\m\id = moveId
			If ml\bef <> Null
				ml\bef\nex = ml\nex
			EndIf
	...
	While ml\nex <> Null ;fast forward to end
		ml = ml\nex
	Wend
	
	While ml\m <> hold ;rewind back to hold
		ml = ml\bef
	Wend
	...
	If name = "en passant"
		...
		If b\prevMoves\m <> Null
			If b\prevMoves\m\id = 101 Or b\prevMoves\m\id = 102 ;white/black pawn double-moves
				...
				;DebugLog "Double move!"
				;DebugLog b\prevMoves\destCell\id
				...
				If b\prevMoves\destCell\dullhorse[leftSide] <> Null ;the cell exists
					;DebugLog "east"
					;DebugLog b\prevMoves\destCell\dullhorse[2]\id
...
Function checkAllSpecials(b.board, undoSpecial=0)
	...
	While speshes <> Null ;loop through the board's specials
		...
		checkSpecials(speshes\s\name, b) ;check the special moves
		
		If speshes\nex = Null
		;	While speshes\bef <> Null ;rewind at the end
		;		speshes = speshes\bef
		;	Wend
			Exit
		EndIf
