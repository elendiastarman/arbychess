...
Function initialize.game()
	...
	success = import(b, "chess_checkTest.txt") ;gonna try it with chess
...
Function run(g.game)
	...
	While Not KeyHit(1)
		...
		;	target.cell = fetchCellById(38)
		;	If cellAttacked(g\b, target, g\b\current\p) = 1
		;		DebugLog "Ding!"
		;		target\alt = fetchImageById(g\b\images, 5)
		;		If target\alt <> 0
		;			MaskImage target\alt, 1, 1, 1
		;		EndIf
		;	EndIf
								...
								If allowed\c <> Null
									...
									If allowed\c\p <> Null
										If allowed\c\p\players <> Null
											If playerInList(allowed\c\p\players, g\b\current\p) = 1 ;can't take your own piece
												allowed\c\alt = 0
											EndIf
										EndIf
									EndIf
									...
									;If allowed\mv\id = 
									valid = checkConditions(g\b, allowed)
	...
	If segment = "cell images"
		...
		While segment <> ""
			...
			If Mid$(segment, 1, 1) <> ";" ;exclude comments
				...
				imgList\name = Mid$( segment, pos+2, Instr(segment, ",", pos+1)-pos-3 )
				pos = Instr(segment, ",", pos+1)
				imgList\filename = Mid$( segment, pos+2, Instr(segment, ",", pos+1)-pos-3 )
				
				;DebugLog imgList\name+" "+imgList\filename
				
				imgList\image = LoadImage(imgList\filename)
				DebugLog imgList\image
			EndIf
	...
	If segment = "cells"
		...
		While segment <> ""
			...
			If Mid$(segment, 1, 1) <> ";" ;exclude comments
				...
				;DebugLog Mid$( segment, pos+1, Instr(segment, ",", pos+1)-pos-1 )
				;Stop
...
Function checkConditions(b.board, cml.cellMoveList)
