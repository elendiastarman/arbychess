...
Function run(g.game)
	...
	While Not KeyHit(1)
		...
		If checkInput(g\m)
			...
			;cL.celloplayer = g\b\cells
			;While cL <> Null
			;	If cL\c\p <> Null
			;		If cL\c\p\id = 11 Or cL\c\p\id = 12
			;			pL.playerList = g\b\current\nex
			;			While pL <> g\b\current
			;				If cellAttacked(g\b, cL\c, pL\p)
			;					cL\c\alt = fetchImageById(g\b\images, 5)
			;				EndIf
			;				pL = pL\nex
			;			Wend
			;		EndIf
