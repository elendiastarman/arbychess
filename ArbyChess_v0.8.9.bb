...
Type winCondition
	Field kind$
	Field params[5]
	Field simul.winCondition
	;Field winners.playerList
End Type
...
Type winList
	Field id
	Field wincond.winCondition
	Field wld$
	Field bef.winList, nex.winList
End Type
...
Function initialize.game()
	...
	success = import(b, "chess_wrapTest.txt") ;gonna try it with chess
	;ChangeDir(  CurrentDir$()+"/" +"Hexagonal Chess" )
	;success = import(b, "hexagonalChess_glinski.txt") ;gonna try it with hexagonal chess
	...
	While Not KeyHit(1)
		...
		If checkInput(g\m)
			...
			If g\m\hit = 2 ;undo
				...
				clearCellAlts()
				highlightVulnerable(g\b, g\b\current, "denied")
			EndIf
				...
				If g\m\c <> Null
					...
					If g\m\c\p <> Null
						If playerInList(g\m\c\p\players, g\b\current\p)
							...
							g\m\c\alt = fetchImageByName(g\b\images, "selected")
							...
							While allowed <> Null
								If allowed\c <> Null
									...
									allowed\c\alt = fetchImageByName(g\b\images, "allowed")
									valid = 1
									...
									If valid = 1 ;check for out of check
										...
										vuln.celloplayer = checkVulnerable(g\b, g\b\current)
			...
			ElseIf g\m\selector = 2 Or g\m\selector = 3
				...
				If g\m\selector = 2 ;second selection
					If g\m\c <> Null
						...
						If cml <> Null
							...
							If checkWin(g\b) = 1
								DebugLog "win"
								;DebugLog "checkWin function took "+(MilliSecs()-timing)+" milliseconds"
								
								clearCellAlts()
								highlightVulnerable(g\b, g\b\current\nex, 5)
								display(g\b)
								
								Color 100, 100, 100
								;Text 20, 20, g\b\current\p\name
								Rect 0,0,GraphicsWidth(),46,1
								Color 255, 255, 255
								;Text 20+8*Len(g\b\current\p\name)+8, 20, "WON!"
								Text 20, 20, "Checkmate! "+g\b\current\p\name+" won!"
								Text 20, 33, "Left-click to exit, right-click to undo."
								Flip
				...
				selected = Null
				clearCellAlts()
				highlightVulnerable(g\b, g\b\current, 5)
				g\m\selector = 0
...
Function import(b.board, path$="")
					...
					If pos+1 <= Len(segment) ;if there are players owning this piece
						...
						Else ;comma separated list
							...
							While Instr(segment, ",", pos+1) < Len(segment)
								...
								If c\p\players\p = Null
									c\p\players\p = fetchPlayerById(tempId)
								Else
									;appendPlayer(c\p\players, fetchPlayerById(tempId))
									c\p\players\nex = New playerList
									c\p\players\nex\p = fetchPlayerById(tempId)
									c\p\players\nex\bef = c\p\players
									c\p\players = c\p\players\nex
								EndIf
								DebugLog c\p\players\p\name
							Wend
						EndIf
						
						While c\p\players\bef <> Null
							c\p\players = c\p\players\bef
						Wend
						DebugLog c\p\players\p\name
	...
	If segment = "wld"
		...
		While segment <> "" And Eof(file)=0
			...
			;parsing - id,type$ | kind$,params | kind$,params | etc...
			If Mid$(segment, 1, 1) <> ";" ;exclude comments
				DebugLog segment
				;cond.winCondition = New winCondition
				
				If b\winConds = Null
					b\winConds = New winList
				Else
					b\winConds\nex = New winList
					b\winConds\nex\bef = b\winConds\nex\bef
					b\winConds = b\winConds\nex
				EndIf
				
				Local conds$[1000]
				splitListToArray( split(segment, "|"), conds)
				
				Local dats$[1000]
				splitListToArray( split(conds[1],","), dats)
				
				b\winConds\id = Int(dats[1])
				b\winConds\wld = Mid$( Trim(dats[2]), 2, Len(Trim(dats[2]))-2)
				
				condition.winCondition = Null
				
				For k = 2 To Int(conds[0])
					If condition = Null
						condition = New winCondition
						b\winConds\wincond = condition
					Else
						condition\simul = New winCondition
						condition = condition\simul
					EndIf
					
					splitListToArray( split(conds[k],","), dats)
					
					condition\kind = Mid$( Trim(dats[1]), 2, Len(Trim(dats[1]))-2)
					condition\params[0] = Int(dats[0])-1
					For i = 2 To Int(dats[0])
						condition\params[i-1] = Int(dats[i])
					Next
				Next
...
Function display(b.board)
	...
	Text 20,33,"Left-click to select and move, right-click to undo."
...
Function checkWin(b.board)
	...
	vuln.celloplayer = checkVulnerable(b, b\current\nex)
	...
	Else
		While vuln <> Null
			...
			While cL <> Null And win = 1
				If cL\c\p <> Null
				
					While vuln\c\p\players <> Null
						If playerInList(cL\c\p\players, vuln\c\p\players\p) And win = 1
						
							playas.playerList = cL\c\p\players
							
							While playas <> Null
								If playas <> b\current
									allowed.cellMoveList = possibleCells(cL\c, cL\c\p\m, b\ramifying, playas\p)
									...
									While allowed <> Null
										...
										vuln2.celloplayer = checkVulnerable(b, b\current\nex)
										If vuln2 = Null
											win = 0
										Else
											While vuln2 <> Null
						...
						If vuln\c\p\players\nex = Null
							Exit
						EndIf
						vuln\c\p\players = vuln\c\p\players\nex
					Wend
					
					While vuln\c\p\players\bef <> Null
						vuln\c\p\players = vuln\c\p\players\bef
					Wend
		...
		If fin\mv <> Null
			...
			While dests1 <> Null
				If dests1\c <> Null
					...
					If moveInList(dests1\c\visits, fin\mv) = 0
						allow = 0
						...
						If allow >= 1
							...
							final\nex\c = dests1\c
							;dests1\c\visited = 1
							appendMove(dests1\c\visits, fin\mv)
			...
			While fin\mv\chainMoves <> Null
				...
				While dests2 <> Null ;loop through destinations
					If dests2\c <> Null
						...
						If moveInList(dests2\c\visits, fin\mv\chainMoves\m) = 0
							allow = 0
							...
							If allow >= 1
								...
								final\nex\c = dests2\c
								;dests2\c\visited = 1
								appendMove(dests2\c\visits, fin\mv\chainMoves\m)
...
Function fetchImageByName(imgList.imageList, name$)
...
Function checkSpecials(spec.special, b.board, undoSpecial=0)
	...
	If spec\name = "en passant" ;;;
		...
		If undoSpecial = 0 ;this is for removing the en passant move if it wasn't used
		
			If b\prevMoves\bef <> Null
				If b\prevMoves\bef\m <> Null
					If b\prevMoves\bef\m\id = 101 Or b\prevMoves\bef\m\id = 102
						cL.celloplayer = b\cells
						While cL <> Null
							If cL\c <> Null
								If cL\c\p <> Null
									If cL\c\p\id = 1
										;DebugLog "white pawn"
										removeMove(cL\c\p\m, 107) ;this works because removeMove does nothing if the move is not found
										removeMove(cL\c\p\m, 108)
									ElseIf cL\c\p\id = 2
										;DebugLog "black pawn"
										removeMove(cL\c\p\m, 105)
										removeMove(cL\c\p\m, 106)
									EndIf
								EndIf
							EndIf
							cL = cL\nex
...
Function checkVulnerable.celloplayer(b.board, playa.playerList) ;returns cells with vulnerable pieces on them
	...
	While cL <> Null
		If cL\c\p <> Null
			If playerInList(cL\c\p\players, playa\p) And cL\c\p\vulnerable = 1
				pL.playerList = playa\nex
				While pL <> playa
...
Function highlightVulnerable(b.board, playa.playerList, highlightImageName)

	vuln.celloplayer = checkVulnerable(b, playa)
	
	While vuln <> Null
		vuln\c\alt = fetchImageById(b\images, highlightImageName)
		If vuln\nex <> Null
			vuln = vuln\nex
			Delete vuln\bef
		Else
