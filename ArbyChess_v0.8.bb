				...
				If g\m\c <> Null
					If g\m\c\p <> Null
						If playerInList(g\m\c\p\players, current)
							...
							allowed = possibleCells(g\m\c, g\m\c\p\m, g\b\ramifying)
...
Function possibleCells.celloplayer(c.cell, moves.movelist, ramifying)
	...
	dests.celloplayer = Null
	d.cell = Null
	
	While fin <> Null ;condition TBD
	
		While fin\mv <> Null

			dests = destinations(fin\c, fin\mv\m\dx, fin\mv\m\dy, ramifying)
			
			While dests <> Null
				If dests\c <> Null
					If dests\c\visited = 0
						
						If (fin\mv\m\options[1] > 0 And dests\c\p = Null) Or (fin\mv\m\options[2] > 0 And dests\c\p <> Null)
							dests\c\visited = 1
							final\nex = New cellMoveList
							final\nex\c = dests\c
							
							If (fin\mv\m\options[1] = 2 And dests\c\p = Null) Or (fin\mv\m\options[2] = 2 And dests\c\p <> Null) ;momentum
								If moveInList(fin\mv\m\nextMoves, fin\mv\m) = 1
									final\nex\mv = New moveList
									final\nex\mv\m = fin\mv\m
								EndIf
							ElseIf (fin\mv\m\options[1] = 3 And dests\c\p = Null) Or (fin\mv\m\options[2] = 3 And dests\c\p <> Null) ;chain
								final\nex\mv = final\mv\m\nextMoves
							EndIf
	...
	f.celloplayer = New celloplayer

	While final\bef <> Null ;rewind cell list
		f\c = final\c
		f\c\visited = 0
		f\bef = New celloplayer
		f\bef\nex = f
		f = f\bef
